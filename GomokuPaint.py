...
import os
import numpy as np
from GomokuConst import rows_default, cols_default, COLORS_ALL, IP
...
class GomokuPainter:
    def __init__(self, h, w, r=rows_default, c=cols_default, ip=IP, equal_interval=True):
        self._height = h
        self._width = w
        self._rows = r
        self._cols = c
        ...
        self._equal_interval = equal_interval
    ...
    def update_board_coords(self):
        ...
        intervals = {k: v[0] / (v[1] + v[2] * 2 - 1) for k, v in paras.items()}
        if self._equal_interval:
            intervals = {k:min(intervals.values()) for k in intervals.keys()}
